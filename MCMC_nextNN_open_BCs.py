...
'''
# @jit(nopython=True)
def runMCMC(Nspins,Beta,Nvmcsteps,verbose,J,J2,Lx,connect):
...
# perform the Monte Carlo steps 
  for ivmc in range(Nvmcsteps):
    ...
    #   Hastings-Metropolis algorithm S0[k,n]-->-S0[k,n]
           ...
           # energies_correlated.append(econf(Lx,J,J2,S0)) # Here we record the energy after each spin flip attempt.
         

#    compute energy every Nsweep updates
     enow = econf(Lx,J,J2,S0)
...
'''
@jit(nopython=True)
def econf(Lx,J,J2,S0,connect =2):
  energy = 0.
  for i in range(Lx):
      for j in range(Lx):
...
RETURN_runMCMC=runMCMC(Nspins,Beta,Nvmcsteps,verbose,J,J2,Lx,connect=2)